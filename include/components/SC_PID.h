...
namespace SC
{
	...
	public:
		...
		SC_PID(SC::SC_PIDConfig PIDCfg, std::string Name = "");
		~SC_PID();

		void Configure(SC::SC_PIDConfig PIDCfg);
		...
		/**
		 * @brief	When the input is true, the input value is wrapped along the defined
		 *			range of the setpoint. By default, this range is [0, 100].
		 */
		void SetContinuousInput(bool cont);
		 ...
		 * 
		 * @param[in]   nPV     Process Variable (PV) that should be used in the calculation
		 * @param[in]   SP      The new SP to target
		 */
		double Calculate(double nPV, double SP);
		 ...
		 */
		double Calculate(double nPV, double nSP, double FB);
	...
	// Members
		...
		SC::SC_Range<double> SCR_Output;
		double SP, PV, CV, CVfb;
		double manRate, trackerCV, manualCV;

		bool enabled, manualMode, inputIsCont;
		bool reversed;
